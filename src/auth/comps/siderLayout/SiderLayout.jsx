...
const SiderLayout = () => {
    ...
    return (
        <div className=' flex w-full '>
            <div className='bg-[#464748] w-fit h-full text-white md:pt-0 pt-28 shadow-lg shadow-gray-500 overflow-y-scroll fixed top-0 z-50'>
                <h2 className='p-5 font-bold text-2xl hidden md:flex'>Logo</h2>
                <ul className='px-3 md:px-5 flex flex-col md:gap-5 gap-10 pt-5 relative'>
                    {dashboardOption.map((item) => (
                        <Link to={item.path} key={item.path}>
                            <li className={` flex items-center gap-5 p-2 rounded text-xl hover:bg-[#5e5f60] ${item.name === 'settings' ? ' fixed bottom-0 pl-2 md:pr-8 bg-[#464748] w-fit' : ''}`} key={item.path}>{item.icon} <span className=' text-lg opacity-70 hidden md:flex'>{item.name}</span></li>
                        </Link>
                    ))}
                </ul>
            </div>
